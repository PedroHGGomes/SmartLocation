...
<img width="1889" height="862" alt="image" src="https://github.com/user-attachments/assets/ac1af46e-042a-4e2c-84ec-1be9568cf81d" />
...
Endpoints:
- **Liveness**: `GET /health/live` — verifica se o processo está vivo.
- **Readiness**: `GET /health/ready` — checa dependências (EF/Oracle/Disco) antes de receber tráfego.
- **Aggregated**: `GET /health` — visão completa.

Formato de resposta: **JSON** com `status`, `totalDuration` e `entries` por checagem.
...
```json
"ConnectionStrings": {
  "ConexaoOracle": "User Id=...;Password=...;Data Source=oracle.fiap.com.br:1521/orcl"
},
"HealthChecks": {
  "DiskRoot": "C",
  "MinFreeGB": 1
...
<img width="506" height="481" alt="image" src="https://github.com/user-attachments/assets/1a008a2c-2cea-48f4-aed7-2bff6f291f21" />
